...
export function LadderPowerSection({
  ...
  const hasMotor = Boolean(motor);
  const hasGround = Boolean(ground || mains?.params.supplyType === "wye" || (mains?.params.supplyType !== "delta" && mains));

  const motorCx = x + width - 90;
  const motorCy = busY2;
  const busEndX = hasMotor ? motorCx - 32 : x + width - 60;

  const inlineKeys: Array<"disconnect" | "breaker" | "fuses" | "contactor" | "overload"> = [];
  if (disconnect) inlineKeys.push("disconnect");
  if (breaker) inlineKeys.push("breaker");
  if (fuses) inlineKeys.push("fuses");
  if (contactor) inlineKeys.push("contactor");
  if (overload) inlineKeys.push("overload");

  const startX = x + 95;
  const availableSpan = Math.max(0, (busEndX - 45) - startX);
  const itemStep = inlineKeys.length > 0 ? Math.min(115, availableSpan / inlineKeys.length) : 0;

  const positions: Record<string, number> = {};
  inlineKeys.forEach((key, idx) => {
    positions[key] = startX + idx * itemStep;
  });

  const disconnectX = positions["disconnect"] || (x + 95);
  const breakerX = positions["breaker"] || (x + 205);
  const fusesX = positions["fuses"] || (x + 315);
  const contactorX = positions["contactor"] || (x + 425);
  const overloadX = positions["overload"] || (x + 535);
  ...
  return (
    <g className="ladder-power-section">
      ...
      >
        ⚡ {title} [{voltage}V 3-PHASE AC{hasMotor ? ` / ${power}kW (${Math.round(power * 1.341)} HP)` : " MAIN CIRCUIT"}]
      </text>
      ...
      <g className="ladder-power-bus">
        ...
        <line x1={x + 35} y1={busY1} x2={busEndX} y2={busY1} stroke={busColor1} strokeWidth="2.8" />
        ...
        <line x1={x + 35} y1={busY2} x2={busEndX} y2={busY2} stroke={busColor2} strokeWidth="2.8" />
        ...
        <line x1={x + 35} y1={busY3} x2={busEndX} y2={busY3} stroke={busColor3} strokeWidth="2.8" />
        ...
        {hasGround && (
          <>
            ...
            <line x1={x + 35} y1={busY4} x2={hasMotor ? motorCx : busEndX} stroke="#10b981" strokeWidth="1.8" strokeDasharray="6 3" />
          </>
        )}
      </g>
